...
void handleBlogsAPIError(DioError err, ErrorInterceptorHandler handler, Dio dio,
    AppRouter appRouter, IUserDbService userDbService) async {
  ...
  switch (err.type) {
    ...
    case DioErrorType.response:
      if (err.response != null) {
        if (err.response!.statusCode == 404) {
          ...
          await userDbService.deleteAccessToken();
          Global.showCustomSnackBar(
            title: "Error",
            message: "token expired!! you have been logged out!!",
            contentType: ContentType.failure,
          );
          appRouter.replaceAll([const SigninRouter()]);
          return;
        } else {
          handler.resolve(Response(
    ...
    case DioErrorType.cancel:
      handler.resolve(Response(
          data: 'user cancelled blogs request',
          requestOptions: err.requestOptions));
      break;
